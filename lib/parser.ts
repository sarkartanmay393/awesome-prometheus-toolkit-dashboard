...
import type { Exporter, Group, Rule, RuleYML } from "@/types";
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        jsonData?.groups?.[0]?.rules?.forEach((rule, i) => {
          monitoringItems.push({
            ...
            alerts: 1,
            code: convertRuleToYamlString(rule),
          });
        });
        // });
      }
...
export const convertRuleToYamlString = (
  rule: RuleYML["groups"][0]["rules"][0]
): string => {
  const yamlObject = {
    alert: rule.alert,
    expr: rule.expr,
...
// Function to read and parse the main YAML file
      ...
      exporters: service.exporters.map((exporter) => {
        ...
        return {
          ...
          rules: exporter.rules?.map((rule) => {
            const correspondingRule = exactRulesFromRulesBySlug.find(r => r?.annotations?.summary.includes(rule.name));
            return {
              name: rule.name,
